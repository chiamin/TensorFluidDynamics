...
function apply_original_tci_vortex(
    ...
    ymin=-21.0,
    ymax=21.0,
    R=9,  # 2^9 = 512 points per dimension (finer grid)
    tol=1e-7,
    maxbonddim=300,
    verbosity=1,
...
# Apply original TCI
# Configuration for finer grid (512x512 = 2^9)
R = 9  # 2^9 = 512 points per dimension (finer grid than v1's 256x256)

# Apply original TCI (may need higher maxbonddim for finer grid)
...
# Apply adaptive patched TCI (may need higher maxbonddim for finer grid)
...
function save_errors_to_hdf5_vortex(errors_original, errors_patched, xs, ys, fs,
                                    ...
                                    output_file::String; tol=1e-7, maxbonddim=30, R=9, patch_bounds=nothing)
