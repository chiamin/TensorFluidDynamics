...
def lanczos_ground_state(H, psi0, k=20, dtype=complex):
    ...
    """
    T, vecs = lanczos_iterations(H, psi0, k, dtype=dtype)
    E, v = np.linalg.eigh(T)
    result = vecs @ v[:, 0]
    if abs(np.linalg.norm(result) - 1.) > 1.e-6:
        warnings.warn("poorly conditioned lanczos. Maybe a non-hermitian H?")
    return E[0], result

def lanczos_expm_multiply (H, psi0, dt, k=20, dtype=float):
    ...
    T, vecs = lanczos_iterations(H, psi0, k, dtype=dtype)
    #print(T)
    v0 = np.zeros(T.shape[0],dtype=dtype)
    v0[0] = 1.
    vt = expm(dt*T) @ v0
    result = vecs @ vt
    if abs(np.linalg.norm(result) - 1.) > 1.e-8:
        warnings.warn("poorly conditioned lanczos. Maybe a non-hermitian H?")
    return result

def lanczos_iterations(H, psi0, k=20, dtype=float):
    ...
    #print(psi0)
    psi = H @ psi0
    alpha = T[0, 0] = np.inner(psi0.conj(), psi)
    ...
    for i in range(1, k):
        ...
        alpha = np.inner(vecs[-1].conj(), psi)
        psi = psi - alpha * vecs[-1]
        T[i, i] = alpha
